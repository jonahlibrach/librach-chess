...
Board::Board(Xwindow* gf, std::string& boardStructureFile){
    gfx = gf;
    board = NULL;
    rowMap = NULL;
    columnMap = NULL;
...
void Board::draw() {
    ...
    for (int i = 0; i < 8; i++) { // 1...8
        gfx->fillRectangle(0, (i*cell) + 1, cell, cell, Xwindow::White);
        std::string word(1, '8' - i);
        gfx->drawString(cell/2, (i*cell) + 1 + (cell/2), word);
    }
    for (int j = 0; j < 8; j++) { // a...h
        gfx->fillRectangle(((j+1)*cell) + 1, 549 - cell, cell, cell, Xwindow::White);
        std::string word(1, 'a' + j);
        gfx->drawString(cell + (j*cell) + 1 + (cell/2), 550-(cell/2), word);
    }
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            draw(i, j);
...
void Board::draw(int row, int col) {
    int cell = 550/(9);
    int color;
    if ((row + col) % 2 == 0) { //white // for the tile
        color = Xwindow::Lightbrown;
    } else { //black
        color = Xwindow::Darkbrown;
    }
    gfx->fillRectangle((col+1)*cell+1, row*cell+1, cell, cell, color);
    char c = squareChar(getSquare(row, col));
    if (c == '_'){
        c = ' ';
    }
    std::string word(1, c);
    ...
    gfx->drawString(((col+1)*cell) + 1 + (cell/2),
                    (row*cell) + 1 + (cell/2), word, color);
}